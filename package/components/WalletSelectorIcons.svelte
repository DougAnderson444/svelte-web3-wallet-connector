<script>import { fade } from 'svelte/transition';
import { createEventDispatcher } from 'svelte';
export let icon; // props
const dispatch = createEventDispatcher();
</script>

<button
	class="cursor-pointer flex flex-auto relative opacity-80 h-full items-center justify-center
text-inherit bg-none border-none m-0 p-0 text-base bg-inherit"
	on:keypress={() => dispatch('click', 'detail value')}
	on:click={() => dispatch('click', 'detail value')}
>
	<div class="img-container">
		{#if icon === 'close'}
			<div transition:fade={{ delay: 100, duration: 100 }}>
				<svg
					class="svg"
					xmlns="http://www.w3.org/2000/svg"
					enable-background="new 0 0 24 24"
					viewBox="0 0 24 24"
					fill="currentColor"
					><rect fill="none" height="24" width="24" /><path
						d="M3,3v18h18V3H3z M17,15.59L15.59,17L12,13.41L8.41,17L7,15.59L10.59,12L7,8.41L8.41,7L12,10.59L15.59,7L17,8.41L13.41,12 L17,15.59z"
					/></svg
				>
			</div>
		{/if}
		{#if icon === 'launch'}
			<div transition:fade={{ delay: 100, duration: 100 }}>
				<svg class="svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
					><path d="M0 0h24v24H0z" fill="none" /><path
						d="M19 19H5V5h7V3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z"
					/></svg
				>
			</div>
		{/if}
		{#if icon === 'plug'}
			<div transition:fade={{ delay: 100, duration: 100 }}>
				<svg class="svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
					><path d="M0 0h24v24H0z" fill="none" /><path
						d="M16.01 7L16 3h-2v4h-4V3H8v4h-.01C7 6.99 6 7.99 6 8.99v5.49L9.5 18v3h5v-3l3.5-3.51v-5.5c0-1-1-2-1.99-1.99z"
					/></svg
				>
			</div>
		{/if}
		{#if icon === 'unplug'}
			<div transition:fade={{ delay: 100, duration: 100 }}>
				<svg class="svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
					><path d="M0 0h24v24H0V0z" fill="none" /><path
						d="M18 14.49V9c0-1-1.01-2.01-2-2V3h-2v4h-4V3H8v2.48l9.51 9.5.49-.49zm-1.76 1.77L7.2 7.2l-.01.01L3.98 4 2.71 5.25l3.36 3.36C6.04 8.74 6 8.87 6 9v5.48L9.5 18v3h5v-3l.48-.48L19.45 22l1.26-1.28-4.47-4.46z"
					/></svg
				>
			</div>
		{/if}
	</div>
	<slot />
</button>

<style>.relative {
		position: relative;
}.m-0 {
		margin: 0px;
}.flex {
		display: flex;
}.h-full {
		height: 100%;
}.flex-auto {
		flex: 1 1 auto;
}.cursor-pointer {
		cursor: pointer;
}.items-center {
		align-items: center;
}.justify-center {
		justify-content: center;
}.border-none {
		border-style: none;
}.bg-inherit {
		background-color: inherit;
}.bg-none {
		background-image: none;
}.p-0 {
		padding: 0px;
}.text-base {
		font-size: 1rem;
		line-height: 1.5rem;
}.text-inherit {
		color: inherit;
}.opacity-80 {
		opacity: 0.8;
}.img-container {
		margin: calc(var(--spacing) / 2);
		width: 1.6em;
		height: 1.6em;
		position: relative;
	}.svg {
		width: 100%;
		height: 100%;
		top: 0;
		bottom: 0;
		left: 0;
		right: 0;
		position: absolute;
	}@media(min-width:640px){.sm\:flex{display:flex;}.sm\:text-base{font-size:1rem;line-height:1.5rem;}}@media(min-width:768px){.md\:flex{display:flex;}.md\:p-2{padding:0.5rem;}}</style>
